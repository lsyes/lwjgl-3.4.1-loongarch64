...
#if defined(__x86_64__) || defined(_M_X64) || defined(__aarch64__) || defined(_M_ARM64) || defined(__loongarch_lp64)
...
#ifdef SIMD_LOONGARCH_SX
SIMD_TARGET
inline v16u8 lsxDecodeShuffleMask(unsigned char mask0, unsigned char mask1)
{
	v16u8 sm0 = __lsx_vld(&kDecodeBytesGroupShuffle[mask0], 0);
	v16u8 sm1 = __lsx_vld(&kDecodeBytesGroupShuffle[mask1], 0);
	v16u8 sm1off = __lsx_vreplgr2vr_b(kDecodeBytesGroupCount[mask0]);

	v16u8 sm1r = __lsx_vadd_b(sm1, sm1off);

	return __lsx_vilvl_d((v2i64)sm1r, (v2i64)sm0);
}

SIMD_TARGET
inline void lsxMoveMask(v16u8 mask, unsigned char& mask0, unsigned char& mask1)
{
	// magic constant found using z3 SMT assuming mask has 8 groups of 0xff or 0x00
	const uint64_t magic = 0x000103070f1f3f80ull;

	v2i64 mask2 = (v2i64)mask;

	mask0 = (unsigned char)(((uint64_t)(mask2[0]) * magic) >> 56);
	mask1 = (unsigned char)(((uint64_t)(mask2[1]) * magic) >> 56);
}
...
inline const unsigned char* decodeBytesGroupSimd(const unsigned char* data, unsigned char* buffer, int hbits)
{
	...
	{
		v16u8 result;
		result = __lsx_vxor_v(result, result);

		__lsx_vst(result, buffer, 0);
	...
	case 6:
	{
		...
		// Load first 4 bytes of data into low word of vector
		v16u8 sel2 = (v16u8)__lsx_vinsgr2vr_w((v4i32)__lsx_vxor_v((v16u8)sel2, (v16u8)sel2), *(const int*)data, 0);
		v16u8 rest = __lsx_vld(data + 4, 0);

		// Unpack 2-bit nibbles: double interleave with shifts
		v16u8 sel22 = __lsx_vilvl_b((v16i8)sel2, (v16i8)__lsx_vsrli_b(sel2, 4));
		v16u8 sel2222_low = __lsx_vilvl_b((v16i8)sel22, (v16i8)__lsx_vsrli_b(sel22, 2));
		v16u8 sel2222_high = __lsx_vilvh_b((v16i8)sel22, (v16i8)__lsx_vsrli_b(sel22, 2));
		v16u8 sel_low = __lsx_vand_v(sel2222_low, __lsx_vreplgr2vr_b(3));
		v16u8 sel_high = __lsx_vand_v(sel2222_high, __lsx_vreplgr2vr_b(3));
		v16u8 sel = __lsx_vilvl_d((v2i64)sel_high, (v2i64)sel_low);

		v16u8 mask = __lsx_vseq_b(sel, __lsx_vreplgr2vr_b(3));
		unsigned char mask0, mask1;
		lsxMoveMask(mask, mask0, mask1);

		v16u8 shuf = lsxDecodeShuffleMask(mask0, mask1);
		v16u8 result = __lsx_vbitsel_v(sel, __lsx_vshuf_b((v16i8)rest, (v16i8)rest, shuf), mask);
	...
	case 7:
	{
		...
		// Load first 8 bytes of data into low 64 bits
		v16u8 sel4 = __lsx_vld(data, 0);
		v16u8 rest = __lsx_vld(data + 8, 0);

		// Unpack 4-bit nibbles: interleave with shifts, extract low 8 bytes
		v16u8 sel44 = __lsx_vilvl_b((v16i8)sel4, (v16i8)__lsx_vsrli_b(sel4, 4));
		v16u8 sel = __lsx_vand_v(sel44, __lsx_vreplgr2vr_b(15));

		v16u8 mask = __lsx_vseq_b(sel, __lsx_vreplgr2vr_b(15));
		unsigned char mask0, mask1;
		lsxMoveMask(mask, mask0, mask1);
	...
	case 5:
	{
		v16u8 rest = __lsx_vld(data + 2, 0);

		unsigned char mask0 = data[0];
		unsigned char mask1 = data[1];

		v16u8 shuf = lsxDecodeShuffleMask(mask0, mask1);
		v16u8 result = __lsx_vshuf_b((v16i8)rest, (v16i8)rest, shuf);
...
#ifdef SIMD_LOONGARCH_SX
SIMD_TARGET
inline void transpose8(v16u8& x0, v16u8& x1, v16u8& x2, v16u8& x3)
{
	v16u8 t0 = __lsx_vilvl_b((v16i8)x1, (v16i8)x0);
	v16u8 t1 = __lsx_vilvh_b((v16i8)x1, (v16i8)x0);
	v16u8 t2 = __lsx_vilvl_b((v16i8)x3, (v16i8)x2);
	v16u8 t3 = __lsx_vilvh_b((v16i8)x3, (v16i8)x2);

	x0 = (v16u8)__lsx_vilvl_h((v8i16)t2, (v8i16)t0);
	x1 = (v16u8)__lsx_vilvh_h((v8i16)t2, (v8i16)t0);
	x2 = (v16u8)__lsx_vilvl_h((v8i16)t3, (v8i16)t1);
	x3 = (v16u8)__lsx_vilvh_h((v8i16)t3, (v8i16)t1);
}

SIMD_TARGET
inline v16u8 unzigzag8(v16u8 v)
{
	v16u8 xl = (v16u8)__lsx_vneg_b((v16i8)__lsx_vand_v(v, __lsx_vreplgr2vr_b(1)));
	v16u8 xr = __lsx_vsrli_b(v, 1);

	return __lsx_vxor_v(xl, xr);
}

SIMD_TARGET
inline v16u8 unzigzag16(v16u8 v)
{
	v16u8 xl = (v16u8)__lsx_vneg_h((v8i16)__lsx_vand_v(v, (v16u8)__lsx_vreplgr2vr_h(1)));
	v16u8 xr = (v16u8)__lsx_vsrli_h((v8i16)v, 1);

	return __lsx_vxor_v(xl, xr);
}

SIMD_TARGET
inline v16u8 rotate32(v16u8 v, int r)
{
	return (v16u8)__lsx_vor_v(
	    (v16u8)__lsx_vsll_w((v4i32)v, __lsx_vreplgr2vr_w(r)),
	    (v16u8)__lsx_vsrl_w((v4u32)v, __lsx_vreplgr2vr_w(32 - r)));
}

template <int Channel>
SIMD_TARGET inline v16u8 lsxRebase(v16u8 npi, v16u8 r0, v16u8 r1, v16u8 r2, v16u8 r3)
{
	switch (Channel)
	{
	case 0:
	{
		v16u8 rsum = __lsx_vadd_b(__lsx_vadd_b(r0, r1), __lsx_vadd_b(r2, r3));
		// sum across bytes: low 8 + high 8, then 4 byte halves
		v16u8 rsumx = __lsx_vadd_b(rsum, (v16u8)__lsx_vbsrl_v(rsum, 8));
		return __lsx_vadd_b(__lsx_vadd_b(npi, rsumx), (v16u8)__lsx_vbsrl_v(rsumx, 4));
	}
	case 1:
	{
		v8i16 rsum = __lsx_vadd_h(__lsx_vadd_h((v8i16)r0, (v8i16)r1), __lsx_vadd_h((v8i16)r2, (v8i16)r3));
		// sum across halfwords
		v8i16 rsumx = __lsx_vadd_h(rsum, (v8i16)__lsx_vbsrl_v((v16u8)rsum, 8));
		return (v16u8)__lsx_vadd_h(__lsx_vadd_h((v8i16)npi, rsumx), (v8i16)__lsx_vbsrl_v((v16u8)rsumx, 4));
	}
	case 2:
	{
		v16u8 rsum = __lsx_vxor_v(__lsx_vxor_v(r0, r1), __lsx_vxor_v(r2, r3));
		v16u8 rsumx = __lsx_vxor_v(rsum, (v16u8)__lsx_vbsrl_v(rsum, 8));
		return __lsx_vxor_v(__lsx_vxor_v(npi, rsumx), (v16u8)__lsx_vbsrl_v(rsumx, 4));
	}
	default:
		return npi;
	}
...
#if defined(SIMD_SSE) || defined(SIMD_AVX) || defined(SIMD_NEON) || defined(SIMD_WASM) || defined(SIMD_LOONGARCH_SX)
SIMD_TARGET
...
#ifdef SIMD_LOONGARCH_SX
#define TEMP v16u8
#define PREP() v16u8 pi = (v16u8)__lsx_vinsgr2vr_w((v4i32)__lsx_vxor_v((v16u8)pi, (v16u8)pi), *reinterpret_cast<const int*>(last_vertex), 0)
#define LOAD(i) v16u8 r##i = __lsx_vld(buffer + j + i * vertex_count_aligned, 0)
#define GRP4(i) t0 = (v16u8)__lsx_vshuf4i_w((v4i32)r##i, 0x00), t1 = (v16u8)__lsx_vshuf4i_w((v4i32)r##i, 0x55), t2 = (v16u8)__lsx_vshuf4i_w((v4i32)r##i, 0xAA), t3 = (v16u8)__lsx_vshuf4i_w((v4i32)r##i, 0xFF)
#define FIXD(i) t##i = pi = Channel == 0 ? (v16u8)__lsx_vadd_b(pi, t##i) : (Channel == 1 ? (v16u8)__lsx_vadd_h((v8i16)pi, (v8i16)t##i) : (v16u8)__lsx_vxor_v(pi, t##i))
#define SAVE(i) *reinterpret_cast<int*>(savep) = __lsx_vpickve2gr_w((v4i32)t##i, 0), savep += vertex_size
#endif
...
#elif defined(SIMD_LATENCYOPT) && defined(SIMD_LOONGARCH_SX)
		pi = lsxRebase<Channel>(npi, r0, r1, r2, r3);
#else
		(void)npi;
#endif
...
#elif defined(SIMD_SSE) || defined(SIMD_AVX) || defined(SIMD_NEON) || defined(SIMD_WASM) || defined(SIMD_LOONGARCH_SX)
